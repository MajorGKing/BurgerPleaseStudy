...
public class Table : UnlockableBase
{
    ...
    Define.ETableState _tableState = Define.ETableState.None;
    public Define.ETableState TableState
    {
    ...
    public bool IsOccupied
    {
        get
        {
            ...
            return TableState != Define.ETableState.None;
        }
    }
    ...
    {
        if (TableState == Define.ETableState.Reserved)
        {
            // 손님이 모두 착석하기 기다린다.
            ...
            TableState = Define.ETableState.Eating;
        }
        else if (TableState == Define.ETableState.Eating)
        {
			...
			foreach (GuestController guest in Guests)
            {
				...
				{
					Managers.Resource.Destroy(guest.gameObject);
				});
            }
